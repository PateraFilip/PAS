...
############################
# Krabicovy graf
boxplot(hp,col="yellow",border="red",main="Krabicovy graf",
        ylab="Sila vozu")
...
pasazeri <- Cars93$Passengers
(ac<-table(pasazeri))
(kac<-cumsum(ac))
(rc<-round(prop.table(table(pasazeri)),2))
(krc<-cumsum(rc))
data.frame(cbind("n(i)"=ac,"N(i)"=kac,"f(i)"=rc,"F(i)"=krc))
...
hist(Cars93$Length,col="skyblue",border="darkblue",main="Histogram",
     xlab="Delka vozidla",ylab="Absolutni cetnosti")
