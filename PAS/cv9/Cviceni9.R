...
syst <- Stulong$syst1
hist(syst,col="azure", border="darkblue", main="Histogram pro syst. tlak", xlab="tlak")
descdist(syst ,discrete=FALSE, boot=1000)
(fit1 <- fitdist(syst,"weibull"))
(fit2 <- fitdist(syst,"gamma"))
...
data.frame(distr=c("Normal","Logis","Exp"),
           AIC=c(fit1$aic,fit2$aic,fit3$aic),
           BIC=c(fit1$bic,fit2$bic,fit3$bic)
)
...
#skutečná střední hodnota se nachází v intervalu od 113.43 do 179.95 s pravděpodobností 99%
